...
export class ValidationRules {
  ...
  static realNumberValidator(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.toString().match(/^-{0,1}\d*\.{0,1}\d+$/)
    ) {
      return null
    } else {
  ...
  static decimalValidator(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.toString().match(/^\d+(\.\d{1,2})?$/)
    ) {
      return null
    } else {
      return { invalidNumber: true }
  ...
  static latitudeValidator(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.match(/^-?([1-8]?[1-9]|[1-9]0)\.{1}\d{1,6}/)
    ) {
      return null
    } else {
  ...
  static longitudeValidator(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.match(/^-?([1]?[1-7][1-9]|[1]?[1-8][0]|[1-9]?[0-9])\.{1}\d{1,6}/)
    ) {
      return null
    } else {
  ...
  static onlyAlphaNumeric(control: AbstractControl): ValidationErrors {
    ...
    else return { invalidAlphaNumeric: true }
  }
  ...
  static onlyMACAddress(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.match(/^[0-9a-f]{1,2}([\.:-])(?:[0-9a-f]{1,2}\1){4}[0-9a-f]{1,2}$/gim)
    ) {
      return null
    } else {
  ...
  static dailCodeValidator(control: AbstractControl): ValidationErrors {
    if (
      !control.value ||
      !control.value.length ||
      control.value.match(/^\*[0-9\*#]*[0-9]+[0-9\*#]*#$/)
    ) {
      return null
    } else {
  ...
  static validateStartNumber(controlName: string, ...number): ValidationErrors {
    return (formGroup: FormGroup) => {
      ...
      if (control.value) {
        ...
        if (index >= 0) control.setErrors({ InvalidNumberStart: number })
      } else {
  ...
  static validateStartNumberFromOne(controlName: string, ...number): ValidationErrors {
    return (formGroup: FormGroup) => {
      ...
      if (control.value) {
        let index = number.indexOf(parseInt(control.value[0]))
        if (index < 0) control.setErrors({ InvalidNumberStartFromOne: number })
      } else {
        control.setErrors(null)
      }
    }
  ...
  static validateStartNumberFromTwo(controlName: string, ...number): ValidationErrors {
    return (formGroup: FormGroup) => {
      ...
      if (control.value) {
        let index = number.indexOf(parseInt(control.value[0]))
        if (index < 0) control.setErrors({ InvalidNumberStartFromTwo: number })
      } else {
        control.setErrors(null)
      }
    }
  ...
  static greateThanValidator(
    duration_call_min: string,
    duration_call_max: string,
    calls_min: string,
    calls_max: string,
    delay_min: string,
    delay_max: string,
    sleep_min?: string,
    sleep_max?: string,
    sleep_calls_min?: string,
    sleep_calls_max?: string,
  ): ValidationErrors {
    return (formGroup: FormGroup) => {
      ...
      if (sleep_calls_minControl && sleep_calls_maxControl) {
        if (
          (sleep_calls_minControl.errors && !sleep_calls_minControl.errors.lessThen) ||
          (sleep_calls_maxControl.errors && !sleep_calls_maxControl.errors.greaterThen)
        )
          return
        else if (
          sleep_calls_minControl.value &&
          sleep_calls_maxControl.value &&
      ...
      if (sleep_minControl && sleep_maxControl) {
        if (
          (sleep_minControl.errors && !sleep_minControl.errors.lessThen) ||
          (sleep_maxControl.errors && !sleep_maxControl.errors.greaterThen)
        )
          return
        else if (
          sleep_minControl.value &&
          sleep_maxControl.value &&
      ...
      if (
        (duration_call_minControl.errors && !duration_call_minControl.errors.lessThen) ||
        (duration_call_maxControl.errors && !duration_call_maxControl.errors.greaterThen)
      )
        return
      else if (
        duration_call_minControl.value &&
        duration_call_maxControl.value &&
      ...
      if (
        (calls_minControl.errors && !calls_minControl.errors.lessThen) ||
        (calls_maxControl.errors && !calls_maxControl.errors.greaterThen)
      )
        return
      else if (
        calls_minControl.value &&
        calls_maxControl.value &&
      ...
      if (
        (delay_minControl.errors && !delay_minControl.errors.lessThen) ||
        (delay_maxControl.errors && !delay_maxControl.errors.greaterThen)
      )
        return
      else if (
        delay_minControl.value &&
        delay_maxControl.value &&
  ...
  static greaterThenValidator(minControlName: string, maxControlName: string): ValidationErrors {
    return (formGroup: FormGroup) => {
      ...
      if (
        minControl.value &&
        maxControl.value &&
        parseInt(minControl.value) > parseInt(maxControl.value)
      )
        minControl.setErrors({ greaterThen: true })
      else minControl.setErrors(null)
    }
  }

  static atleastOnePhoneNumber(
    controlToNumber: string,
    controlFromNumber: string,
    matchingControlName: string,
    bypassControlName: string,
  ): ValidationErrors {
    return (formGroup: FormGroup) => {
